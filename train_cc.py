...
class CCTrainer(BaseTrainer):
    def __init__(self, *args, **kwargs):
        super(CCTrainer, self).__init__(*args, **kwargs)
    ...
    def train_one_epoch(self):
        ...
        for inx, data in pbar_train:
            ct32, ct128, label = data['ct32'].to(self.device), data['ct64'].to(self.device), data['label'].to(self.device)
            cls = self.model(ct128, ct32)
...
def main(args, path):
    ...
    print(device)
    from src.nets import CCNet
    model = CCNet(num_classes=args.num_classes, model_depth=args.rd)
    ...
    train_dataset = LungDataset(train_info, opt.dataset, use_ct32=True, use_ct64=True)
    val_dataset = LungDataset(val_info, opt.dataset, use_ct32=True, use_ct64=True, phase='val')
