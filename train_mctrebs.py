...
class MCTREBSTrainer(BaseTrainer):
    def __init__(self, *args, **kwargs):
        super(MCTREBSTrainer, self).__init__(*args, **kwargs)
...
def main(args, path):
    ...
    print(device)
    # from src.nets2 import MCREBSNet, ViTBase
    from src.tmss import TMSSNet as MYNET
    model = MYNET(num_classes=args.num_classes)
    ...
    train_dataset = LungDataset(train_info, opt.dataset, use_ct32=True, use_ct128=False, use_radiomics=False, use_cli=True, use_bbox=False, use_seg=False)
    val_dataset = LungDataset(val_info, opt.dataset, use_ct32=True, use_ct128=False, use_radiomics=False, use_cli=True, use_bbox=False, use_seg=False)
    ...
    trainer = MCTREBSTrainer(model=model,
                         optimizer=optimizer,
                         scheduler=scheduler,
                         device=device,
