...
class LungDataset(Dataset):
    def __init__(self,
        ...
        if self.use_radiomics:
            ...
            self.radiomics = pd.merge(self.df[['uid']], self.radiomics, how='left', on=['uid'])
            self.radiomics = self.radiomics.fillna(0)
            radiomics_features = self.radiomics.drop('uid', axis=1)
    ...
    def __getitem__(self, i):
        ...
        if self.use_ct64:
            ct64, seg = self.get_ct_64(i)
        if self.use_ct128:
            ct128, seg = self.get_ct_128(i)
        if self.use_ct256:
            ct256, seg = self.get_ct_256(i)
        if self.use_bbox:
        ...
        res = {'label': label, 'uid': uid, 'ct32': ct32, 'ct64': ct64, 'ct128': ct128,'ct256': ct256,
            'clinical': clinical, 'bbox': bbox, 'mask': mask, 'slice': slice, 'seg': seg, 'radiomic': radiomic}
        return res

    def get_ct_32(self, i):
        ct_path = os.path.join(self.data_dir, self.conf['img_dir_32'], f'{self.uids[i]}.nii.gz')
        self.get_nii_file(ct_path)
        return self.get_nii_file(ct_path)
    def get_ct_64(self, i):
        ct_path = os.path.join(self.data_dir, self.conf['img_dir_64'], f'{self.uids[i]}.nii.gz')
        self.get_nii_file(ct_path)
        return self.get_nii_file(ct_path)
    def get_ct_128(self, i):
        seg = 0
        ct_path = os.path.join(self.data_dir, self.conf['img_dir_128'], f'{self.uids[i]}.nii.gz')
        if not os.path.exists(ct_path):
            ct_path = os.path.join(self.data_dir, self.conf['img_dir_128'], f'{self.uids[i]}.npy')
        if self.use_seg:
    ...
    def get_bbox(self, i):
        ...
        bbox = torch.tensor(bbox).unsqueeze(-1)
        return bbox
    ...
    def get_radiomic(self, i):
        values = torch.tensor(self.radiomics.iloc[i].drop(labels=['uid']).values.tolist(), dtype=torch.float)
        return values.unsqueeze(-1)
...
if __name__ == '__main__':
    train_info, val_info = split_pandas('../configs/dataset_001.json')
    train_dataset = LungDataset(train_info, '../configs/dataset_001.json', use_seg=True, use_ct128=True, use_radiomics=True)
    val_dataset = LungDataset(val_info, '../configs/dataset_001.json', phase='val', use_ct32=True)
    ...
    for res in train_loader:
        print(res['radiomic'].shape, res['radiomic'])
        break
        # pass
