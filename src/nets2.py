...
class RadiomicsTransformer(nn.Module):
    def __init__(self, num_features, num_layers, num_heads):
        ...
        # self.decoder = nn.Linear(num_features, 1)  # 假设是一个回归问题
...
class PositionalEncoding(torch.nn.Module):
    def __init__(self, d_model, max_len=5000):
        ...
        if d_model % 2 == 0:
            ...
            # 当d_model为奇数时，复制最后一个cosine结果以填充最后一个维度
...
if __name__ == '__main__':
    ...
    data = {'ct32' : ct32, 'ct128' : ct128, 'radiomic' : radiomic, 'clinical' : EHR, 'bbox' : bbox, 'seg' : seg}
    # # net = BCENet()
