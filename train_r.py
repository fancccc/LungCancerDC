...
class SBCETrainer(BaseTrainer):
    ...
    def train_one_epoch(self):
        ...
        for inx, data in pbar_train:
            radiomics, label = (data['radiomic'].to(self.device), data['label'].to(self.device))
            cls = self.model(radiomics)
...
def main(args, path):
    ...
    print(device)
    from src.nets2 import RadiomicsNet
    model = RadiomicsNet(num_classes=args.num_classes)
    ...
    train_dataset = LungDataset(train_info, opt.dataset, use_radiomics=True)
    val_dataset = LungDataset(val_info, opt.dataset, use_radiomics=True, phase='val')
