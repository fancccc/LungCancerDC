...
def main(args, path):
    ...
    print(device)
    from src.nets2 import RadiomicsNet, CliNet
    # model = RadiomicsNet(num_classes=args.num_classes)
    model = CliNet(num_classes=args.num_classes)
    ...
    train_dataset = LungDataset(train_info, opt.dataset, use_radiomics=False, use_cli=True)
    val_dataset = LungDataset(val_info, opt.dataset, use_radiomics=False, use_cli=True, phase='val')
