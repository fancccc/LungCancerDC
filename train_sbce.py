...
class SBCETrainer(BaseTrainer):
    def __init__(self, *args, **kwargs):
        super(SBCETrainer, self).__init__(*args, **kwargs)
    ...
    def train_one_epoch(self):
        ...
        for inx, data in pbar_train:
            ct128, bbox, ehr, seg, label = (data['ct128'].to(self.device), data['bbox'].to(self.device),
                                            data['clinical'].to(self.device), data['seg'].to(self.device), data['label'].to(self.device))
            cls = self.model(ct128, bbox, ehr, seg)
    ...
    def evaluate(self):
        ...
        with torch.no_grad():
            ...
            for inx, data in pbar_test:
                ct128, bbox, ehr, seg, label = (data['ct128'].to(self.device), data['bbox'].to(self.device),
                                                data['clinical'].to(self.device), data['seg'].to(self.device),
...
def main(args, path):
    ...
    print(device)
    from src.nets2 import SBCENet
    model = SBCENet(num_classes=args.num_classes)
    ...
    train_dataset = LungDataset(train_info, opt.dataset, use_ct128=True, use_bbox=True, use_cli=True, use_seg=True)
    val_dataset = LungDataset(val_info, opt.dataset, use_ct128=True, use_bbox=True, use_cli=True, use_seg=True, phase='val')
    ...
    trainer = SBCETrainer(model=model,
                         optimizer=optimizer,
                         scheduler=scheduler,
                         device=device,
