...
from text_encoder import load_clip_to_cpu, TextEncoder, PromptLearner
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, args=None):
        ...
        classnames = ['adenocarcinoma in situ', 'minimally invasive adenocarcinoma', 'invasive adenocarcinoma']
        attrnames = ['gender', 'age',
                     'long diameter', 'short diameter', 'round or oval shape', 'irregular',
                     'Lobulation', 'Nodule sign', 'Air bubble sign', 'Cavity', 'Calcification',
                     'Bronchial sign', 'Clear margin', 'Smooth and sharp', 'Spiculation', 'Pointed angle',
                     'Fibrous stripe', 'Vascular convergence sign', 'Pleural indentation',
                     'position top right', 'position low right', 'position right middle',
                     'position top left', 'position low leftp',
        ...
        if args.language:
            ...
            if mode == 'train':
                ...
                for batch_i, (pts_i, attr_pts_i, imf_i, label_i, weights_i) in enumerate(zip(
                    ...
                    # print(imf_i.shape, text_feature.shape)
                    l_i = F.cross_entropy(l_i, torch.cat([label_i.unsqueeze(0) for j in range(l_i.shape[0])], dim=0))
...
if __name__ == '__main__':
    ...
    args.language = True
    model = ResNet18(num_classes=3, args=args)
    image = torch.randn(1, 32, 32, 32)
    # cls_labels = ['adenocarcinoma in situ']
    cls_labels = torch.tensor(1, dtype=torch.long).unsqueeze(0)
    weights = torch.tensor([0.5]).unsqueeze(0)
    # print(cls_labels.shape, weights.shape)
    output = model(image, args, label=cls_labels, weights=weights)
    print(output)
