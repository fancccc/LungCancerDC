...
class BCETrainer(BaseTrainer):
    def __init__(self, *args, **kwargs):
        super(BCETrainer, self).__init__(*args, **kwargs)
    ...
    def train_one_epoch(self):
        ...
        for inx, data in pbar_train:
            ct128, bbox, ehr, label = data['ct128'].to(self.device), data['bbox'].to(self.device), data['clinical'].to(self.device), data['label'].to(self.device)
            cls = self.model(ct128, bbox, ehr)
    ...
    def evaluate(self):
        ...
        with torch.no_grad():
            ...
            for inx, data in pbar_test:
                ct128, bbox, ehr, label = data['ct128'].to(self.device), data['bbox'].to(self.device), data[
...
def main(args, path):
    ...
    print(device)
    from src.nets2 import BCENet
    model = BCENet(num_classes=args.num_classes)
    ...
    train_dataset = LungDataset(train_info, opt.dataset, use_ct128=True, use_bbox=True, use_cli=True)
    val_dataset = LungDataset(val_info, opt.dataset, use_ct128=True, use_bbox=True, use_cli=True, phase='val')
